...
@cli.command()
@click.option('-d', '--host', type=str, help='host to scan for', required=True)
@click.option('-p', '--ports', type=str, help=f'define ports (separate by "{default_split_by}") [None]', default=None)
@click.option('-udp', is_flag=True, help='enables udp port scan instead of tcp')
@click.option('-o', '--options', type=str, help=f'options to scan with [see help]', default=None)
...
def nmap(ctx: Context, host, ports, udp, options, options_append, rate, silent_mode, silent):
    ...
    service: HackService = ctx.service
    try:
        defaults = ['-sS', '-sV', '-O', f'-T{silent_mode}', '-PE', '--open', '-sC',
                    '--script=vuln', '--script=discovery', '-vv']
        options = service.utils.define_option_list(default_options=defaults,
                                                   options=options, options_append=options_append,
                                                   default_split_by=default_split_by)

        service.nmap(host=host, port=ports, udp=udp, options=options, rate=rate, silent=silent)
    except KeyboardInterrupt as k:
        logging.log(logging.DEBUG, f'process interupted! ({k})')
        sys.exit(5)
    except Exception as e:
...
@cli.command()
@click.option('-d', '--host', type=str,
              help='host to scan for', required=True)
@click.option('-m', '--mode', type=click.Choice(['dir', 'vhost', 'fuzz', 'dns', 'bak']),
              help='type to scan for [dir]', default='dir')
@click.option('-t', '--threads', type=int,
              help='threads to use [10]', default=10)
@click.option('-w', '--wordlist', type=str,
              help='wordlist to use')
@click.option('-o', '--options', type=str,
              help='options to scan with [see help]', default=None)
@click.option('-oa', '--options_append', is_flag=True,
...
def gobuster(ctx: Context, host, mode, threads, wordlist, options, options_append):
    ...
    try:
        ...
        service.gobuster(host=host, type=mode, threads=threads, w_list=wordlist, options=options)
    except KeyboardInterrupt as k:
        logging.log(logging.DEBUG, f'process interupted! ({k})')
        sys.exit(5)
    except Exception as e:
...
def recon(ctx: Context, domain, org, mode, threads, depth, nameserver):
    ...
    service: HackService = ctx.service
    try:
        service.recon(domain=domain, org=org, mode=mode, threads=threads, depth=depth, ns=nameserver)
    except KeyboardInterrupt as k:
        logging.log(logging.DEBUG, f'process interupted! ({k})')
        sys.exit(5)
    except Exception as e:
