import logging
import os
import re
import sys
from pathlib import Path
from typing import Any, List, Union

import validators
import verboselogs
import yaml

from ..utils.config import settings
from ..utils.defaultLogBanner import log_runBanner
from ..utils.utilsFolderHelper import create_service_folder, get_user_path
from ..utils.utilsHelper import geo, uri_validator
from ..utils.utilsProcessHelper import run_command_output_loop

# from libnmap.objects.host import NmapHost
# from libnmap.objects.report import NmapReport
# from libnmap.parser import NmapParser, NmapParserException
# from libnmap.process import NmapProcess
default_split_by: str = ','


# ------------------------------------------------------------------------------
#
#
#
# ------------------------------------------------------------------------------
class HackService:

    def __init__(self):
        '''
            hack service
        '''
        logging.log(logging.DEBUG, 'hack-service is initiated')

    # --------------------------------------------------------------------------
    #
    #
    #
    # --------------------------------------------------------------------------

    def clone_page(self, host: str, ssl_verify: bool = True) -> None:
        '''
            ...
        '''
        service_name: str = 'page cloner'
        log_runBanner(service_name)
        path: str = create_service_folder(f'page', host)
        options = []
        if ssl_verify is False:
            options.append('--no-check-certificate')
        run_command_output_loop(
            f'clone page {host}',
            [
                ['wget', '-r', '-nHp', host, '-P', path] + options,
                ['tee', f'{path}/page_clone.log'],
            ],
        )
        logging.log(
            verboselogs.SUCCESS,
            f'[*] {service_name} Done! View the log reports under {path}/',
        )

    # --------------------------------------------------------------------------
    #
    #
    #
    # --------------------------------------------------------------------------

    def recon(
        self,
        domain: Union[str, None],
        org: Union[str, None],
        mode: str = 'gospider',
        threads: int = 10,
        depth: int = 2,
        ns: str = '1.1.1.1',
    ) -> None:
        '''
            ...
        '''
        # check if needed values for module are set
        if mode == 'amass_org':
            if org is None:
                logging.log(logging.WARNING, f'mode "{mode}" need org to be set')
                sys.exit(2)
        else:
            if domain is None:
                logging.log(logging.WARNING, f'mode "{mode}" need domain to be set')
                sys.exit(2)
        # some function need a valid url, check it first
        if mode == 'hakrawler' and domain is not None:
            if not uri_validator(domain):
                logging.log(logging.WARNING, f'domain "{domain}" is not valid')
                sys.exit(2)
        service_name: str = 'RECON'
        log_runBanner(service_name)
        path: str = create_service_folder(f'scan/recon', domain)
        # set/create conf file for subfinder and censys
        sources = []
        if mode == 'subfinder' or mode == 'censys':
            logging.log(logging.DEBUG, 'Create subfinder conf with keys...')
            subfinder_config_path: str = f'{get_user_path()}/.config/subfinder'
            subfinder_config_file = 'config.yaml'
            # sources.append('bufferover')
            # sources.append('sitedossier')
            sources.append('censys')
            sources.append('shodan')
            # sources.append('Binaryedge')
            # sources.append('certspotter')
            # sources.append('Chaos')
            # sources.append('DnsDB')
            sources.append('github')
            # sources.append('Intelx')
            # sources.append('passivetotal')
            # sources.append('Recon.dev')
            # sources.append('Robtex')
            # sources.append('SecurityTrails')
            # sources.append('Spyse')
            # sources.append('Threatbook')
            sources.append('virustotal')
            # sources.append('Zoomeye')
            data = {
                'sources': sources,
                'all-sources': sources,
                'recursive': sources,
                'censys': [
                    f'{settings.SECRET_CENSYS_USERNAME}:{settings.SECRET_CENSYS_SECRET}'
                ],
                'shodan': [f'{settings.SECRET_SHODAN_API_KEY}'],
                # 'virustotal': [f'{TODO}'],
                # 'passivetotal': [f'{TODO},{TODO}'],
                # 'securitytrails': [f'{TODO}'],
                # 'binaryedge': [],
                # 'certspotter': [],
                # 'chaos': [],
                # 'dnsdb': [],
                # 'github': [],
                # 'intelx': [],
                # 'recon': [],
                # 'robtex': [],
                # 'spyse': [],
                # 'threatbook': [],
                # 'urlscan': [],
                # 'zoomeye': [],
            }
            Path(subfinder_config_path).mkdir(parents=True, exist_ok=True)
            with open(f'{subfinder_config_path}/{subfinder_config_file}', 'w') as file:
                yaml.dump(data, file)
                logging.log(
                    logging.DEBUG,
                    f'... {subfinder_config_path}/{subfinder_config_file} ...',
                )
                logging.log(logging.DEBUG, '... subfinder conf with keys created')
        options_1 = ['-t', str(threads), '--recursive', '-v', '-oJ', '-nW']
        options_2 = [
            '-t',
            str(threads),
            '--recursive',
            '-v',
            '-oJ',
            '-nW',
            '--sources',
            ','.join(sources),
        ]
        if mode == 'gospider' and domain is not None:
            run_command_output_loop(
                f'recon {mode}',
                [
                    [
                        'gospider',
                        '-s',
                        domain,
                        '-o',
                        f'{path}/gospider',
                        '-c',
                        str(threads),
                        '-d',
                        str(depth),
                        '--other-source',
                        '--include-subs',
                    ],
                    ['tee', f'{path}/gospider.log'],
                ],
            )
        elif mode == 'hakrawler' and domain is not None:
            run_command_output_loop(
                f'recon {mode}',
                [['echo', domain], ['hakrawler', '-d', str(depth), '-subs', '-t',
                                    str(threads), '-insecure'], ['tee', f'{path}/hakrawler.log']],
                # -h 'Cookie: foo=bar;Authorization: token'
            )
        elif mode == 'subfinder' and domain is not None:
            run_command_output_loop(
                f'recon {mode}',
                [
                    [
                        'subfinder',
                        '-d',
                        domain,
                        '-o',
                        f'{path}/subfinder_rec',
                        '-r',
                        ns,
                    ] +
                    options_1,
                    ['httpx'],
                    ['tee', f'{path}/subfinder_rec.log'],
                ],
            )
        elif mode == 'subfinder_api' and domain is not None:
            run_command_output_loop(
                f'recon {mode}',
                [
                    [
                        'subfinder',
                        '-d',
                        domain,
                        '-o',
                        f'{path}/subfinder_censys',
                        '-r',
                        ns,
                    ] +
                    options_2,
                    ['tee', f'{path}/subfinder_censys.log'],
                ],
            )
        elif mode == 'amass_whois' and domain is not None:
            run_command_output_loop(
                f'recon {mode}',
                [
                    [
                        'amass',
                        'intel',
                        '-d',
                        domain,
                        '-whois',
                        '-r',
                        ns,
                        '-o',
                        f'{path}/amass_whois',
                    ],
                    ['tee', f'{path}/amass_whois.log'],
                ],
            )
        elif mode == 'amass_org' and org is not None:
            run_command_output_loop(
                f'recon {mode}',
                [
                    [
                        'amass',
                        'intel',
                        '-org',
                        org,
                        '-r',
                        ns,
                        '-o',
                        f'{path}/amass_org',
                    ],
                    ['tee', f'{path}/amass_org.log'],
                ],
            )
        elif mode == 'passive' and domain is not None:
            run_command_output_loop(
                f'recon {mode}',
                [
                    [
                        'amass',
                        'enum',
                        '-passive',
                        '-d',
                        domain,
                        '-r',
                        ns,
                        '-o',
                        f'{path}/amass_passive',
                    ],
                    ['tee', f'{path}/amass_passive.log'],
                ],
            )
        elif mode == 'active' and domain is not None:
            run_command_output_loop(
                f'recon {mode}',
                [
                    [
                        'amass',
                        'enum',
                        '-active',
                        '-src',
                        '-ip',
                        '-brute',
                        '-min-for-recursive',
                        str(depth),
                        '-d',
                        domain,
                        '-r',
                        ns,
                        '-o',
                        f'{path}/amass_active',
                    ],
                    ['tee', f'{path}/amass_active.log'],
                ],
            )
        elif mode == 'gau' and domain is not None:
            run_command_output_loop(
                f'recon {mode}',
                [
                    ['gau', '--subs', domain],
                    ['cut', '-d', '/', '-f', '3'],
                    ['sort', '-u'],
                    ['tee', f'{path}/gau.log'],
                ],
            )
        elif mode == 'theHarvester' and domain is not None:
            sources = [
                'baidu',
                'bufferoverun',
                'crtsh',
                'hackertarget',
                'otx',
                'projecdiscovery',
                'rapiddns',
                'sublist3r',
                'threatcrowd',
                'trello',
                'urlscan',
                'vhost',
                'virustotal',
                'zoomeye',
            ]
            for source in sources:
                run_command_output_loop(
                    f'recon {mode}',
                    [
                        [
                            'theHarvester',
                            '-d',
                            domain,
                            '-b',
                            source,
                            '-f',
                            f'theHarvester_{source}',
                        ],
                        ['jq', '-r', '.hosts[]', '2>/dev/null'],
                        ['cut', "-d':'", '-f', '1'],
                        ['sort', '-u'],
                        ['tee', f'{path}/theHarvester_{source}.log'],
                    ],
                )
        logging.log(
            verboselogs.SUCCESS,
            f'[*] {service_name} Done! View the log reports under {path}/',
        )

    # --------------------------------------------------------------------------
    #
    #
    #
    # --------------------------------------------------------------------------

    def dns(
        self,
        host: str,
        ns: Union[str, None] = None,
        record_type: Union[str, None] = None,
        port: int = 53,
        is_subdomain: bool = False,
    ) -> None:
        '''
            ...
        '''
        service_name: str = 'DNS/DIG'
        log_runBanner(service_name)
        path: str = create_service_folder(f'scan/dig', host)
        nameserver = []
        if ns is not None:
            if not ns.startswith('@'):
                ns = f'@{ns}'
            nameserver = [ns]
        mode_plus = ['+answer', '+nocomments', '+multi']  # '+noall',
        mode_subdomain = []
        if is_subdomain is True:
            mode_subdomain = ['a']
        mode_test_01 = ['', 'ANY', 'TXT', 'A', 'NS', 'MX', 'CNAME', 'AXFR', 'SOA']
        if record_type is not None:
            mode_test_01 = [record_type]
        mode_test_02 = ['', 'dkmi._', '_dmarc.']
        for test_01 in mode_test_01:
            for test_02 in mode_test_02:
                file_suffix_1 = f'_{test_01}' if test_01 and len(test_01) > 0 else ''
                file_suffix_2 = f'_{test_02}' if test_02 and len(test_02) > 0 else ''
                cur_record = [test_01] if test_01 and len(test_01) > 0 else []
                run_command_output_loop(
                    'dig',
                    [
                        ['dig'] +
                        mode_subdomain +
                        cur_record +
                        mode_plus +
                        ['-p', str(port)] +
                        nameserver +
                        [f'{test_02}{host}'],
                        ['tee', f'{path}/dig_dns{file_suffix_1}{file_suffix_2}.log'],
                    ],
                )
        logging.log(
            verboselogs.SUCCESS,
            f'[*] {service_name} Done! View the log reports under {path}/',
        )
        service_name = 'DNS/HOST'
        log_runBanner(service_name)
        path = create_service_folder(f'scan/host', host)
        run_command_output_loop(
            'whois', [['whois', host], ['tee', f'{path}/whois.log']]
        )
        run_command_output_loop(
            'host', [['host', '-aRR', host], ['tee', f'{path}/host.log']]
        )
        logging.log(
            verboselogs.SUCCESS,
            f'[*] {service_name} Done! View the log reports under {path}/',
        )

    def domain(self, domain: str) -> None:
        '''
            ...
        '''
        service_name: str = 'DOMAIN'
        log_runBanner(service_name)
        path: str = create_service_folder(f'scan/domain', domain)
        mode_types = ['subdomains', 'tlds', 'all']
        for types in mode_types:
            run_command_output_loop(
                'domain',
                [
                    ['curl', '-s', f'https://sonar.omnisint.io/{types}/{domain}'],
                    ['jq', '-r', '.[]'],
                    ['sort', '-u'],
                    ['tee', f'{path}/domain_{types}.log'],
                ],
            )

    def tls(self, domain: str, port: int = 443) -> None:
        '''
            ...
        '''
        service_name: str = 'TLS'
        log_runBanner(service_name)
        path: str = create_service_folder(f'scan/tls', domain)
        # TODO: check param domain for port or http
        create_domain = f'{domain}:{port}'
        run_command_output_loop(
            'openssl [1/5]',
            [
                ['openssl', 's_client', '-connect', create_domain, '-showcerts'],
                ['tee', f'{path}/openssl_info_cert.log'],
            ],
        )
        run_command_output_loop(
            'openssl [2/5]',
            [
                ['openssl', 's_client', '-connect', create_domain],
                ['tee', f'{path}/openssl_info.log'],
            ],
        )
        run_command_output_loop(
            'openssl [3/5]',
            [
                [
                    'openssl',
                    's_client',
                    '-connect',
                    create_domain,
                    '</dev/null 2>/dev/null',
                ],
                ['openssl', 'x509', '-noout', '-in', '/dev/stdin', '-text'],
                ['tee', f'{path}/openssl_text.log'],
            ],
        )
        run_command_output_loop(
            'openssl [4/5]',
            [
                [
                    'openssl',
                    's_client',
                    '-connect',
                    create_domain,
                    '</dev/null 2>/dev/null',
                ],
                ['openssl', 'x509', '-fingerprint', '-noout', '-in', '/dev/stdin'],
                ['tee', f'{path}/openssl_fingerprint.log'],
            ],
        )
        run_command_output_loop(
            'openssl [5/5]',
            [
                [
                    'openssl',
                    's_client',
                    '-ign_eof',
                    '2>/dev/null',
                    "<<<$'HEAD / HTTP/1.0\r\n\r'",
                    '-connect',
                    create_domain,
                ],
                ['openssl', 'x509', '-noout', '-text', '-in', '-'],
                ['grep', 'DNS'],
                ['sed', '-e', 's|DNS:|\n|g', '-e', 's|^\*.*||g'],
                ['tr', ' -d', ','],
                ['sort', '-u'],
                ['tee', f'{path}/openssl_fingerprint.log'],
            ],
        )
        run_command_output_loop(
            'cert/crt.sh',
            [
                ['curl', '-s', f'https://crt.sh/?q={domain}&output=json'],
                ['jq', '-r', '.[] | "\(.name_value)\n\(.common_name)"'],
                ['sort', '-u'],
                ['tee', f'{path}/cert_crt.sh.log'],
            ],
        )
        logging.log(
            verboselogs.SUCCESS,
            f'[*] {service_name} Done! View the log reports under {path}/',
        )

    # --------------------------------------------------------------------------
    #
    #
    #
    # --------------------------------------------------------------------------

    def nmap(
        self,
        host: str,
        udp: bool = True,
        port: Union[str, None] = None,
        options: List[Any] = [],
        rate: int = 10000,
        path: Union[str, None] = None,
        silent: bool = False,
    ) -> None:
        '''
            ...
        '''
        service_name: str = 'NMAP'
        log_runBanner(service_name)
        path = create_service_folder(f'scan/nmap', host) if path is None else path
        hosts: Union[str, List[str], None] = host.split(' ')
        ports: Union[str, List[str], None] = None  # port
        port_range_scan = ['-p-']  # '-p-' | '-F-' | '--top-ports=1000'
        if port is not None:
            port_range_scan = ['-p', port]
        mode_decoy = []
        t_scan = 4
        if silent:
            mode_decoy = ['-D', 'RND:5']
            rate = 100
            t_scan = 1
            for option in options:
                if '-T' in option:
                    option = f'-T{t_scan}'
        options_script_args = []
        # TODO: ... check host for public without manipulate...
        host_to_check = host if host.startswith('http') else f'http://{host}'
        # TODO: ...
        if validators.url(host_to_check, public=True):
            logging.log(logging.DEBUG, "domain is a public host, try add script-args")
            options_script_args_extend = []
            if settings.SECRET_SHODAN_API_KEY is not None:
                options_script_args_extend.append(
                    f'shodan-api.apikey={settings.SECRET_SHODAN_API_KEY}'
                )
            if settings.GEO_LICENSE_KEY is not None:
                options_script_args_extend.append(f'ip-geolocation.maxmind_db={geo()}')
            if len(options_script_args_extend) > 0:
                options_script_args.append('--script-args')
                options_script_args.append(','.join(options_script_args_extend))
        else:
            logging.log(
                logging.WARNING,
                'if you want to add script-args, you need to provide a public valid host',
            )
        options_privileged = ['--privileged']
        options_improve_scan = [f'--min-rate={rate}', f'-T{t_scan}']
        # nmap --privileged  -Pn -n -sn -PE
        options_host_scan = [
            '-Pn', '-n', '-sn', '-PE'
        ] + options_improve_scan + mode_decoy
        # nmap --privileged -Pn -n --disable-arp-ping -p-
        options_port_scan = [
            '-Pn', '-n', '--disable-arp-ping'
        ] + options_improve_scan + mode_decoy + port_range_scan
        options_port_udp_scan = options_port_scan + ['-sU', '-sT', '--max-retries=1']
        options_output_format = ['-oX', f'{path}/inital.xml', '-oA', f'{path}/inital']
        if settings.LOGGING_VERBOSE is not None and settings.LOGGING_VERBOSE >= 3:
            options.append('-d')
        options_full_scan = [
            '--reason'
        ] + options_improve_scan + mode_decoy + options_output_format + options_script_args + options
        # nmap_report_hosts = self.utils.nmap_process('nmap host up scan', hosts, options_host_scan)
        # hosts = [host.address for host in nmap_report_hosts.hosts if host.is_up()]
        # logging.log(logging.DEBUG, hosts)
        if isinstance(hosts, List):
            hosts = run_command_output_loop(
                'nmap host-up scan',
                [['nmap'] + options_privileged + options_host_scan + hosts, ['grep', 'report for'],
                    ['cut', '-d', ' ', '-f5'], ['sort'], ['uniq'], ['tr', '\\n', ' '], ['sed', 's/ $//']],
                # TODO: check grep when scan between ip and domain
            )
        else:
            logging.log(
                logging.WARNING,
                'host str was not parsed correct to list, check your typed host',
            )
            hosts = None
        if hosts is not None:
            hosts = hosts.split(' ')
            # SAVE host to file
            with open(f'{path}/hosts.lst', 'w') as file:
                file.write('\n'.join(hosts))
            # RUN scan for ports, if not specified
            if ports is None:
                # if udp:
                #     nmap_report_ports = self.utils.nmap_process('nmap udp ports scan', hosts, options_port_udp_scan)
                # else:
                #     nmap_report_ports = self.utils.nmap_process('nmap tcp ports scan', hosts, options_port_scan)
                # ports_host_tmp = []
                # [ports_host_tmp.extend(host.get_ports()) for host in nmap_report_ports.hosts if host.is_up()]
                # ports = sorted(set((str(p[0]) for p in ports_host_tmp)))
                # logging.log(logging.DEBUG, ports)
                if udp:
                    ports = run_command_output_loop(
                        'nmap udp ports scan',
                        [
                            settings.USE_SUDO +
                            ['nmap'] +
                            options_privileged +
                            options_port_udp_scan +
                            hosts,
                            ['grep', '^[0-9]'],
                            ['cut', '-d', '/', '-f', '1'],
                            ['sort'],
                            ['uniq'],
                            ['tr', '\\n', default_split_by],
                            ['sed', f's/{default_split_by}$//'],
                        ],
                    )
                else:
                    ports = run_command_output_loop(
                        'nmap tcp ports scan',
                        [
                            settings.USE_SUDO +
                            ['nmap'] +
                            options_privileged +
                            options_port_scan +
                            hosts,
                            ['grep', '^[0-9]'],
                            ['cut', '-d', '/', '-f', '1'],
                            ['sort'],
                            ['uniq'],
                            ['tr', '\\n', default_split_by],
                            ['sed', f's/{default_split_by}$//'],
                        ],
                    )
            # --initial-rtt-timeout 50ms --max-rtt-timeout 100ms
            # nmap -T1 --min-rate=100 -O -n -sn -Pn -sA
            if isinstance(ports, str):
                ports = ports.split(default_split_by)
                # SAVE ports to file
                with open(f'{path}/ports.lst', 'w') as file:
                    file.write('\n'.join(ports))
                # RUN full scan for information's
                run_command_output_loop(
                    'nmap scan',
                    [
                        settings.USE_SUDO +
                        ['nmap'] +
                        options_privileged +
                        options_full_scan +
                        ['-p', ','.join(ports)] +
                        hosts,
                        ['tee', f'{path}/nmap.log'],
                    ],
                )
                # nmap_report = self.utils.nmap_process('nmap scan', hosts, options_full_scan+['-p', ','.join(ports)], safe_mode=False)
                if os.path.isfile(f'{path}/inital.xml'):
                    run_command_output_loop(
                        'nmap convert xls',
                        [
                            [
                                'nmap-converter',
                                f'{path}/inital.xml',
                                '-o',
                                f'{path}/inital.xls',
                            ]
                        ],
                    )
                    run_command_output_loop(
                        'nmap convert html',
                        [
                            [
                                'xsltproc',
                                f'{path}/inital.xml',
                                '-o',
                                f'{path}/inital.html',
                            ]
                        ],
                    )
                    logging.log(
                        verboselogs.SUCCESS,
                        f'[*] {service_name} Done! View the log reports under {path}/',
                    )
                else:
                    logging.log(logging.WARNING, '[-] Any error in full nmap scan')
            else:
                logging.log(logging.WARNING, '[-] No ports found')
        else:
            logging.log(logging.WARNING, '[-] No host are reachable')

    def masscan(self, host: str, rate: int = 10000, options: List[str] = []) -> None:
        '''
            ...
        '''
        service_name: str = 'MASSCAN'
        log_runBanner(service_name)
        path: str = create_service_folder(f'scan/masscan', host)
        run_command_output_loop(
            'masscan',
            [
                settings.USE_SUDO +
                ['masscan', host, '-oX', f'{path}/masscan.xml'] +
                options
            ],
        )
        run_command_output_loop(
            'xsltproc',
            [
                [
                    'xsltproc',
                    '-o',
                    f'{path}/final-masscan.html',
                    '/opt/git/nmap-bootstrap-xsl/nmap-bootstrap.xsl',
                    f'{path}/masscan.xml',
                ]
            ],
        )
        ports = run_command_output_loop(
            'cat ports',
            [
                ['cat', f'{path}/masscan.xml'],
                ['grep', 'portid'],
                ['cut', '-d', '\"', '-f', '10'],
                ['sort', '-n'],
                ['uniq'],
                ['paste', '-sd,'],
            ],
        )
        if ports is not None:
            t_scan = 4
            logging.log(
                verboselogs.SUCCESS,
                f'[*] {service_name} Done! View the log reports under {path}/',
            )
            n_options = [
                '-sV', '-O', f'-T{t_scan}', '-PE', '-Pn', '-n', '--open', '-vv'
            ]
            self.nmap(host=host, port=ports, options=n_options, path=path, rate=rate)
        else:
            logging.log(logging.WARNING, '[-] No ports found')

    # TODO: add ffuf
    # ffuf -w ./vhosts -u http://10.129.42.195 -H "HOST: FUZZ.www.inlanefreight.htb"
    # ffuf -recursion -recursion-depth 1 -u http://192.168.10.10/FUZZ -w /opt/useful/SecLists/Discovery/Web-Content/raft-small-directories-lowercase.txt
    # cewl -m5 --lowercase -w wordlist.txt http://192.168.10.10
    # ffuf -w ./folders.txt:FOLDERS,./wordlist.txt:WORDLIST,./extensions.txt:EXTENSIONS -u http://192.168.10.10/FOLDERS/WORDLISTEXTENSIONS

    def gobuster(
        self,
        host: str,
        type: str = 'dir',
        threads: int = 10,
        w_list: Union[str, None] = None,
        options: List[str] = [],
        exclude_length: Union[int, None] = None,
    ) -> None:
        '''
            ...
        '''
        service_name: str = 'GOBUSTER'
        log_runBanner(service_name)
        path: str = create_service_folder(f'scan/gobuster', host)
        # wordlist = f'{settings.HOME_PATH}/.vm_recon/git/SecLists/Discovery/Web-Content/big.txt' if w_list is None else w_list
        wordlist = f'{settings.HOME_PATH}/.vm_recon/git/SecLists/Discovery/Web-Content/raft-medium-words.txt' if w_list is None else w_list
        if exclude_length is not None:
            options + ['--exclude-length', str(exclude_length)]
        if type == 'dir' or type is None:
            wordlist = f'{settings.HOME_PATH}/.vm_recon/git/SecLists/Discovery/Web-Content/big.txt' if w_list is None else w_list
            run_command_output_loop(
                'gobuster dir',
                [
                    [
                        'gobuster',
                        'dir',
                        '-u',
                        host,
                        '-w',
                        wordlist,
                        '-r',
                        '-t',
                        str(threads),
                        '-o',
                        f'{path}/gobuster_dir',
                    ] +
                    options,
                    ['tee', f'{path}/gobuster_dir.log'],
                ],
            )
        elif type == 'vhost':
            wordlist = f'{settings.HOME_PATH}/.vm_recon/git/SecLists/Discovery/DNS/subdomains-top1million-110000.txt' if w_list is None else w_list
            run_command_output_loop(
                'gobuster vhost',
                [
                    [
                        'gobuster',
                        'vhost',
                        '-u',
                        host,
                        '-w',
                        wordlist,
                        '-r',
                        '-t',
                        str(threads),
                        '-o',
                        f'{path}/gobuster_vhost',
                    ] +
                    options,
                    ['tee', f'{path}/gobuster_vhost.log'],
                ],
            )
        elif type == 'fuzz':
            # wordlist = f'{settings.HOME_PATH}/.vm_recon/git/SecLists/Discovery/Web-Content/big.txt' if w_list is None else w_list
            # wordlist = f'{settings.HOME_PATH}/.vm_recon/git/SecLists/Discovery/Web-Content/Apache.fuzz.txt' if w_list is None else w_list
            # wordlist = f'{settings.HOME_PATH}/.vm_recon/git/SecLists/Discovery/Web-Content/ApacheTomcat.fuzz.txt' if w_list is None else w_list
            wordlist = f'{settings.HOME_PATH}/.vm_recon/git/SecLists/Discovery/Web-Content/FatwireCMS.fuzz.txt' if w_list is None else w_list
            # wordlist = f'{settings.HOME_PATH}/.vm_recon/git/SecLists/Discovery/Web-Content/CMS/wordpress.fuzz.txt' if w_list is None else w_list
            # wordlist = f'{settings.HOME_PATH}/.vm_recon/git/SecLists/Discovery/Web-Content/CMS/wp-plugins.fuzz.txt' if w_list is None else w_list
            # wordlist = f'{settings.HOME_PATH}/.vm_recon/git/SecLists/Discovery/Web-Content/CMS/wp-themes.fuzz.txt' if w_list is None else w_list
            run_command_output_loop(
                'gobuster fuzz',
                [
                    [
                        'gobuster',
                        'fuzz',
                        '-u',
                        host,
                        '-w',
                        wordlist,
                        '-r',
                        '-t',
                        str(threads),
                        '-o',
                        f'{path}/gobuster_fuzz',
                    ] +
                    options,
                    ['tee', f'{path}/gobuster_fuzz.log'],
                ],
            )
        elif type == 'dns':
            wordlist = f'{settings.HOME_PATH}/.vm_recon/git/SecLists/Discovery/DNS/subdomains-top1million-110000.txt' if w_list is None else w_list
            run_command_output_loop(
                'gobuster dns',
                [
                    [
                        'gobuster',
                        'dns',
                        '-d',
                        host,
                        '-w',
                        wordlist,
                        '-r',
                        '-t',
                        str(threads),
                        '-o',
                        f'{path}/gobuster_dns',
                    ] +
                    options,
                    ['tee', f'{path}/gobuster_dns.log'],
                ],
            )
        elif type == 'bak':
            wordlist = f'{settings.HOME_PATH}/.vm_recon/git/SecLists/Discovery/Web-Content/big.txt' if w_list is None else w_list
            run_command_output_loop(
                'gobuster bak',
                [
                    [
                        'gobuster',
                        'dir',
                        '-u',
                        host,
                        '-w',
                        wordlist,
                        '-d',
                        '-r',
                        '-t',
                        str(threads),
                        '-o',
                        f'{path}/gobuster_back',
                    ] +
                    options,
                    ['tee', f'{path}/gobuster_back.log'],
                ],
            )
        else:
            logging.log(logging.WARNING, f'gobuster type "{type}" not defined')
        logging.log(
            verboselogs.SUCCESS,
            f'[*] {service_name} Done! View the log reports under {path}/',
        )

    def kitrunner(self, host: str, w_list: Union[str, None] = None) -> None:
        '''
            ...
        '''
        service_name: str = 'KITRUNNER'
        log_runBanner(service_name)
        path: str = create_service_folder(f'scan/kr', host)
        wordlist = '/opt/git/kiterunner/routes.kite' if w_list is None else w_list
        max_connection_per_host = 10
        ignore_length = 34
        run_command_output_loop(
            'kitrunner',
            [
                [
                    'kr',
                    'scan',
                    host,
                    '-w',
                    wordlist,
                    '-A=apiroutes-210228:20000',
                    '-x',
                    str(max_connection_per_host),
                    f'--ignore-length={ignore_length}',
                ],
                ['tee', f'{path}/kr_scan.log'],
            ],
        )
        logging.log(
            verboselogs.SUCCESS,
            f'[*] {service_name} Done! View the log reports under {path}/',
        )

    def sqlmap(
        self,
        host: str,
        data: Union[str, None] = None,
        cookie: Union[str, None] = None,
        method: str = 'POST',
        technique: str = 'U',
        dbms: Union[str, None] = None,
        dbs=False,
        tables=False,
        database_name: Union[str, None] = None,
        tables_name: Union[str, None] = None,
        random_agent=True,
        banner=True,
        parse_errors=True,
        dump=True,
        tamper: Union[str, None] = None,
        level: int = 5,
        risk: int = 3,
        threads: int = 10,
        verbose: int = 3,
    ) -> None:
        '''
            tamper:
                - between
                - charunicodeescape
            technique:
                - B: Boolean-based blind
                - E: Error-based
                - U: Union query-based
                - S: Stacked queries
                - T: Time-based blind
                - Q: Inline queries
        '''
        service_name: str = 'SQLMAP'
        log_runBanner(service_name)
        path: str = create_service_folder(f'scan/sqlmap', host)
        database_name_l = ['-D', database_name] if database_name else []
        tables_name_l = ['-T', tables_name] if tables_name else []
        data = f'--data={data}' if data else ''
        cookie = f'--cookie={cookie}' if cookie else ''
        method = f'--method={method}' if method else ''
        technique = f'--technique={technique}' if technique else ''
        dbms = f'--dbms={dbms}' if dbms else ''
        dbs = '--dbs' if dbs else ''
        tables = '--tables' if tables else ''
        random_agent = '--random-agent' if random_agent else ''
        banner = '--banner' if banner else ''
        parse_errors = '--parse-errors' if parse_errors else ''
        dump = f'--dump' if dump else ''
        tamper = f'--tamper={tamper}' if tamper else ''
        run_command_output_loop(
            'sqlmap',
            [
                [
                    'sqlmap',
                    '-u',
                    host,
                    random_agent,
                    banner,
                    parse_errors,
                    tamper,
                    f'--level={level}',
                    f'--risk={risk}',
                    f'--threads={threads}',
                    '-v',
                    str(verbose),
                    data,
                    cookie,
                    method,
                    technique,
                    dbms,
                    dbs,
                    tables,
                    dump,
                ] +
                database_name_l +
                tables_name_l,
                ['tee', f'{path}/sqlmap.log'],
            ],
        )
        logging.log(
            verboselogs.SUCCESS,
            f'[*] {service_name} Done! View the log reports under {path}/',
        )

    # --------------------------------------------------------------------------
    #
    #
    #
    # --------------------------------------------------------------------------

    def smb(self, hosts: str, ports: str = '139;445') -> None:
        '''
            ...
        '''
        service_name: str = 'SMB'
        log_runBanner(service_name)
        for host in hosts.split(' '):
            for port in ports.split(default_split_by):
                path: str = create_service_folder(f'scan/smb', host)
                # SMBCLIENT ############################################################
                ########################################################################
                run_command_output_loop(
                    'smbclient scan',
                    [
                        ['smbclient', '-N', '-p', str(port).strip(), '-L', f'//{host}'],
                        ['tee', f'{path}/smbclient.log'],
                    ],
                )
                print('')
                print(
                    '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
                )
                logging.log(
                    verboselogs.NOTICE,
                    f'use "smbclient -U anonymous //{host}/..." to check the results',
                )
                logging.log(verboselogs.NOTICE, 'usefull commands "dir,get,put,..."')
                logging.log(
                    verboselogs.NOTICE, f'usefull commands "smbget -R smb://{host}/..."'
                )
                print(
                    '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
                )
                print('')
                # ENUM4LINUX ############################################################
                ########################################################################
                run_command_output_loop(
                    'enum4linux scan',
                    [['enum4linux', host], ['tee', f'{path}/enum4linux.log']],
                )
                print('')
                print(
                    '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
                )
                logging.log(
                    verboselogs.NOTICE,
                    f'use "smbmap -u \'...\' -p \'\' -R -H {host}" to check the results',
                )
                print(
                    '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
                )
                print('')

                logging.log(
                    verboselogs.SUCCESS,
                    f'[*] {service_name} Done! View the log reports under {path}/',
                )
        # NMAP #################################################################
        ########################################################################
        path = create_service_folder(f'scan/smb', hosts)
        smb_options = [
            '--script',
            'smb-vuln-*,smb-os-discovery,smb-enum-shares.nse,smb-enum-users.nse',
        ]
        n_options = [
            '-sV', '-O', '-T4', '-PE', '-Pn', '-n', '--open', '-vv'
        ] + smb_options
        self.nmap(
            host=hosts, port=ports, udp=True, options=n_options, path=path, silent=False
        )

    def rpc(self, hosts: str, ports: str = '111') -> None:
        '''
            ...
        '''
        service_name: str = 'RPC'
        log_runBanner(service_name)
        path: str = create_service_folder(f'scan/rpc', hosts)
        # for host in hosts.split(' '):
        #     for port in ports.split(','):
        #         path:str = create_service_folder(f'scan/smb', host)
        #         # SMBCLIENT ############################################################
        #         ########################################################################
        #         run_command_output_loop('smbclient scan', [
        #             ['smbclient', '-N', '-p', str(port).strip(), '-L', f'//{host}'],
        #             ['tee', f'{path}/smbclient.log']
        #         ])
        #         print('')
        #         print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        #         logging.log(verboselogs.NOTICE, f'use "smbclient //{host}/..." to check the results')
        #         logging.log(verboselogs.NOTICE, 'usefull commands "dir,get,put,..."')
        #         print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        #         print('')
        # logging.log(logging.INFO,f'[*] {service_name} Done! View the log reports under {path}/')
        # NMAP #################################################################
        ########################################################################
        rpc_options = ['--script', 'nfs-ls,nfs-statfs,nfs-showmount']
        n_options = [
            '-sV', '-O', '-T4', '-PE', '-Pn', '-n', '--open', '-vv'
        ] + rpc_options
        self.nmap(
            host=hosts, port=ports, udp=True, options=n_options, path=path, silent=False
        )

    # --------------------------------------------------------------------------
    #
    #
    #
    # --------------------------------------------------------------------------

    def whatweb(self, host: str, silent: int = 3) -> None:
        '''
            ...
        '''
        service_name: str = 'WHATWEB'
        log_runBanner(service_name)
        path: str = create_service_folder(f'scan/whatweb', host)
        run_command_output_loop(
            'whatweb silent',
            [
                [
                    'whatweb',
                    host,
                    '-a',
                    str(silent),
                    '-v',
                    f'--log-verbose={path}/whatweb_v.log',
                    f'--log-json={path}/whatweb_j.log',
                ]
            ],
        )
        run_command_output_loop(
            'wafw00f', [['wafw00f', '-v', host], ['tee', f'{path}/wafw00f.log']]
        )
        logging.log(
            verboselogs.SUCCESS,
            f'[*] {service_name} Done! View the log reports under {path}/',
        )

    def wpscan(self, host: str, silent: bool = False) -> None:
        '''
            ...
        '''
        service_name: str = 'WPSCAN'
        log_runBanner(service_name)
        path: str = create_service_folder(f'scan/wpscan', host)
        mode = ['--plugins-detection', 'aggressive'] if silent == False else [
            '--plugins-detection', 'passive'
        ]
        options = ['--random-user-agent', '-e', 'ap']
        to_file = ['-o', f'{path}/wpscan']
        run_command_output_loop(
            f'wpscan {mode}',
            [
                ['wpscan', '--url', host] + options + to_file + mode,
                ['tee', f'{path}/wpscan.log'],
            ],
        )
        logging.log(
            verboselogs.SUCCESS,
            f'[*] {service_name} Done! View the log reports under {path}/',
        )

    def accounting(
        self,
        domain: Union[str, None] = None,
        username: Union[str, None] = None,
        email: Union[str, None] = None,
    ) -> None:
        '''
            ...
        '''
        service_name: str = 'ACCOUNTING'
        log_runBanner(service_name)
        splitter: Union[str, None] = None
        loop_append: List[Union[str, None]] = [domain, username, email]
        for a in loop_append:
            if a is not None:
                if splitter is None:
                    splitter = a
                else:
                    splitter = f'{splitter}_{a}'
        path: str = create_service_folder(f'scan/accounting', splitter)
        if domain is not None:
            run_command_output_loop(
                f'emailfinder',
                [['emailfinder', '-d', domain], ['tee', f'{path}/emailfinder.log']],
            )
        if username is not None:
            # '--output', f'{path}/sherlock.name'
            cmd_result = run_command_output_loop(
                f'sherlock',
                [
                    ['sherlock', username, '--folderoutput', f'{path}/'],
                    ['tee', f'{path}/sherlock.log'],
                ],
            )
            if cmd_result is not None:
                search_insta = r'(?:.*?)instagram(?:.*?)\/(.*)'
                found_insta = re.findall(search_insta, cmd_result)
                if found_insta is not None:
                    options_sterra = [
                        '-t',
                        'both',
                        '-a',
                        '--descending',
                        '--size',
                        '15',
                        '--all-infos',
                    ]
                    output_sterra = ['-p', f'{path}/sterra', '-f', 'csv']
                    cred_sterra = []
                    if settings.SECRET_INSTAGRAM_SSID is not None:
                        cred_sterra = ['-ssid', settings.SECRET_INSTAGRAM_SSID]
                        for fi in found_insta:
                            cmd_result = run_command_output_loop(
                                f'sterra',
                                [
                                    ['sterra', '-u', fi] +
                                    options_sterra +
                                    output_sterra +
                                    cred_sterra,
                                    ['tee', f'{path}/sterra_{fi}.log'],
                                ],
                            )
        if email is not None:
            pass
        logging.log(
            verboselogs.SUCCESS,
            f'[*] {service_name} Done! View the log reports under {path}/',
        )

    # --------------------------------------------------------------------------
    #
    #
    #
    # --------------------------------------------------------------------------

    def pwn(self, file: str) -> None:
        '''
            ...
        '''
        service_name: str = 'PWN'
        log_runBanner(service_name)
        path: str = create_service_folder(f'pwn/checks', file)
        run_command_output_loop(
            f'pwn file', [['file', file], ['tee', f'{path}/file.log']]
        )
        run_command_output_loop(
            f'pwn strings',
            [['strings', '-n', '10', file], ['tee', f'{path}/strings.log']],
        )
        run_command_output_loop(
            f'pwn checksec',
            [['checksec', '--file', file], ['tee', f'{path}/checksec.log']],
        )
        logging.log(
            verboselogs.SUCCESS,
            f'[*] {service_name} Done! View the log reports under {path}/',
        )


# ------------------------------------------------------------------------------
#
#
#
# ------------------------------------------------------------------------------
